...
app.config.update(
    ...
    SESSION_COOKIE_SAMESITE="Lax",      # CSRF protection while still working in WebView
    SESSION_COOKIE_SECURE=os.environ.get("FLASK_ENV") != "development",  # HTTPS only in production
    PERMANENT_SESSION_LIFETIME=timedelta(days=30),  # stay logged in 30 days
)
...
<script type="application/ld+json">
{
  ...
  "itemListElement": [
    ...
    {"@type":"SiteNavigationElement","position":2,"name":"Log In","url":"{{request.url_root.rstrip('/')}}/login"},
...
<nav>
  <div class="ni">
    ...
    <div class="nl">
      ...
      <a href="/login" class="{% if request.path=='/login' %}act{% endif %}">Log In</a>
...
<div class="bn">
  ...
  <a href="/login" class="{% if request.path=='/login' or request.path=='/member/profile' %}act{% endif %}"><span class="ic">👤</span><span>Log In</span></a>
...
<div style="max-width:400px;margin:80px auto;">
  <div class="card" style="text-align:center;">
    ...
    <h2 style="border:none;">Member Login</h2>
    ...
    return render(tmpl, _title="Log In — Civision Society", nav_logo=get_nav_logo(),
