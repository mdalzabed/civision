...
MEMBER_COOKIE = "cs_member_token"
ADMIN_COOKIE  = "cs_admin_token"
COOKIE_DAYS   = 15
COOKIE_MAX    = COOKIE_DAYS * 24 * 3600

def _set_cookie(response, name, value, max_age=COOKIE_MAX):
...
def start_member_session(mid):
    """Generate token, write to Firestore with verification, return 'mid:token'."""
    import time
    token = secrets.token_hex(32)
    doc_ref = col("members").document(mid)
    # Write token
    doc_ref.set({"session_token": token}, merge=True)
    # Verify it actually wrote (handles Vercel cold-start Firestore latency)
    for _ in range(3):
        doc = doc_ref.get()
        if doc.exists and doc.to_dict().get("session_token") == token:
            break
        time.sleep(0.3)
        doc_ref.set({"session_token": token}, merge=True)
    return f"{mid}:{token}"
...
def get_logged_in_member():
    """Read cookie, look up member directly by ID, compare token — no index needed."""
    cookie = request.cookies.get(MEMBER_COOKIE)
    if not cookie or ":" not in cookie: return None
    try:
        mid, token = cookie.split(":", 1)
    except ValueError: return None
    member = get_member_by_id(mid)
    if not member: return None
    if member.get("status") != "Approved": return None
    if member.get("session_token") != token: return None
    return member

def end_member_session():
    """Revoke the token in Firestore."""
    cookie = request.cookies.get(MEMBER_COOKIE)
    if not cookie or ":" not in cookie: return
    try:
        mid, _ = cookie.split(":", 1)
        col("members").document(mid).update({"session_token": ""})
    except Exception: pass

def start_admin_session():
    """Generate admin token, store in Firestore, return it."""
    token = secrets.token_hex(32)
    col("settings").document("admin_session").set({"token": token}, merge=True)
    return token

def get_logged_in_admin():
    """Validate admin cookie against Firestore token."""
    token = request.cookies.get(ADMIN_COOKIE)
    if not token: return False
    doc = col("settings").document("admin_session").get()
    if doc.exists and doc.to_dict().get("token") == token:
        return True
    return False

def end_admin_session():
    try: col("settings").document("admin_session").set({"token": ""})
    except: pass
...
def admin_required(f):
    @wraps(f)
    def dec(*a, **kw):
        if not get_logged_in_admin(): return redirect(url_for("admin_login"))
        return f(*a, **kw)
    return dec
...
def member_login():
    if request.method == "POST":
        ...
        token = start_member_session(mid)
        # Render profile page directly in the same response as the cookie set
        # so the browser gets both together — fixes Vercel redirect cookie race
        ...
        # Re-fetch member to get latest data
        member = get_member_by_id(mid)
        resp = make_response(render_profile_page(member, club_balance))
        _set_cookie(resp, MEMBER_COOKIE, token)
        return resp
    tmpl = r"""
...
def member_logout():
    ...
    _clear_cookie(resp, MEMBER_COOKIE)
    return resp

def render_profile_page(member, club_balance):
    tmpl = r"""
...
def member_profile():
    ...
    total_expenses  = sum(float(e.get("amount",0) or 0) for e in all_expenses)
    club_balance    = total_collected - total_expenses
    return render_profile_page(member, club_balance)
...
def member_update():
    ...
    if new_pw:
        ...
        new_cookie = start_member_session(mid)
        flash("Profile updated!","s")
        resp = make_response(redirect(url_for("member_profile")))
        _set_cookie(resp, MEMBER_COOKIE, new_cookie)
        return resp
    if updates: update_member(mid, updates)
    flash("Profile updated!","s")
    return redirect(url_for("member_profile"))
...
def admin_login():
    if get_logged_in_admin(): return redirect(url_for("admin_dashboard"))
    if request.method == "POST":
        if check_admin_password(request.form.get("password","")):
            token = start_admin_session()
            flash("Welcome to the Admin Portal.","s")
            resp = make_response(redirect(url_for("admin_dashboard")))
            _set_cookie(resp, ADMIN_COOKIE, token)
            return resp
...
def admin_logout():
    end_admin_session()
    resp = make_response(redirect(url_for("home")))
    _clear_cookie(resp, ADMIN_COOKIE)
    return resp
...
def check_cookie_debug():
    cookies = dict(request.cookies)
    mc = cookies.get(MEMBER_COOKIE, "")
    result = {
        "member_cookie_present": MEMBER_COOKIE in cookies,
        "preview": mc[:30]+"..." if len(mc) > 30 else mc,
        "firestore_token": None,
        "tokens_match": False
    }
    if mc and ":" in mc:
        try:
            mid, token = mc.split(":", 1)
            doc = col("members").document(mid).get()
            if doc.exists:
                fs_token = doc.to_dict().get("session_token","")
                result["firestore_token"] = fs_token[:20]+"..." if len(fs_token) > 20 else fs_token
                result["tokens_match"] = (fs_token == token)
                result["member_status"] = doc.to_dict().get("status","")
        except Exception as e:
            result["error"] = str(e)
