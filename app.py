...
def start_member_session(mid):
    """Generate a token, store in Firestore, return 'mid:token' for the cookie."""
    token = secrets.token_hex(32)
    col("members").document(mid).set({"session_token": token}, merge=True)
    return f"{mid}:{token}"
...
def check_cookie_debug():
    ...
    mc = cookies.get(MEMBER_COOKIE, "NOT FOUND")
    return jsonify({
        "member_cookie_present": MEMBER_COOKIE in cookies,
        "preview": mc[:20]+"..." if len(mc) > 20 else mc
    })
