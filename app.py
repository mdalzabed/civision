...
def start_member_session(mid):
    """Generate a token, store in Firestore, return it so the route can set the cookie."""
    token = secrets.token_hex(32)
    col("members").document(mid).update({"session_token": token})
    return token

def get_logged_in_member():
    """Read the member cookie, validate against Firestore token, return member or None."""
    token = request.cookies.get(MEMBER_COOKIE)
    if not token: return None
    # Find member by token (search all members — token is unique)
    docs = col("members").where("session_token", "==", token).limit(1).stream()
    for doc in docs:
...
def end_member_session():
    """Revoke the token in Firestore. Cookie cleared by the route."""
    token = request.cookies.get(MEMBER_COOKIE)
    if not token: return
    try:
        docs = col("members").where("session_token", "==", token).limit(1).stream()
        for doc in docs:
            doc.reference.update({"session_token": ""})
    except Exception: pass

def start_admin_session():
    """Generate admin token, store in Firestore, return it."""
    token = secrets.token_hex(32)
    col("settings").document("admin_session").set({"token": token})
    return token
...
def member_update():
    ...
    if new_pw:
        ...
        new_token = start_member_session(mid)  # reissue token in Firestore
        updates["session_token"] = new_token
        if updates: update_member(mid, updates)
        flash("Profile updated!","s")
        resp = make_response(redirect(url_for("member_profile")))
        _set_cookie(resp, MEMBER_COOKIE, new_token)
        return resp
    if updates: update_member(mid, updates)
    flash("Profile updated!","s")
    return redirect(url_for("member_profile"))
